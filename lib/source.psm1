...
function Download-Pages {
  ...
  foreach ($item in $sourceData) {
    ...
    Write-Progress -ParentId $ProgressParentId -Id 101 -Activity "Downloading $currentItemPosition / $($sourceData.Count)" -Status "$currentItemPosition / $($sourceData.Count)" -CurrentOperation "$newName..." -PercentComplete $currentProgress
