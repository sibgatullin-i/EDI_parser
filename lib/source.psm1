...
function Download-Pages {
  ...
  foreach ($item in $sourceData) {
    ...
    Write-Progress -ParentId $ProgressParentId -Id ++$ProgressParentId -Activity "$currentItemPosition / $($sourceData.Count)" -Status "Downloading $newName..." -PercentComplete $currentProgress
